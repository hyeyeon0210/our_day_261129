...
(function () {
  ...
  function formatDate(dateStr, timeStr) {
    ...
    return `${year}년 ${month}월 ${date}일 ${day}요일 ${h24}시${minuteStr}`;
  }
  ...
  function initGreeting(images) {
    ...
    const groomImg = (images && images[0]) ? images[0] : 'images/introduce/1.jpg';
    const brideImg = (images && images[1]) ? images[1] : 'images/introduce/2.jpg';
    ...
    // 좌우 배치를 위한 HTML 구조
    const parentsHTML = `
     <div class="profile-container" style="display: flex; justify-content: space-around; gap: 20px; margin-top: 40px; padding: 0 10px;">
      
      <div class="profile-item" style="flex: 1; text-align: center;">
        <div class="profile-circle" style="width: 100px; height: 100px; margin: 0 auto 15px; overflow: hidden; border-radius: 50%; border: 1px solid #eee;">
          <img src="${groomImg}" style="width: 100%; height: 100%; object-fit: cover;">
        </div>
        <div style="font-size: 0.85rem; color: #888; margin-bottom: 4px;">신랑</div>
        <div style="font-size: 1rem; font-weight: bold; margin-bottom: 8px;">${g.name}</div>
        <div style="font-size: 0.8rem; color: #666; line-height: 1.6;">
          ${parentLine(g.father, g.mother, g.fatherDeceased, g.motherDeceased)}의<br> 장남
        </div>
      </div>

      <div class="profile-item" style="flex: 1; text-align: center;">
        <div class="profile-circle" style="width: 100px; height: 100px; margin: 0 auto 15px; overflow: hidden; border-radius: 50%; border: 1px solid #eee;">
          <img src="${brideImg}" style="width: 100%; height: 100%; object-fit: cover;">
        </div>
        <div style="font-size: 0.85rem; color: #888; margin-bottom: 4px;">신부</div>
        <div style="font-size: 1rem; font-weight: bold; margin-bottom: 8px;">${b.name}</div>
        <div style="font-size: 0.8rem; color: #666; line-height: 1.6;">
          ${parentLine(b.father, b.mother, b.fatherDeceased, b.motherDeceased)}의<br> 장녀
        </div>
      </div>
  ...
  function initIntroduce(storyImages) {
    $('#storyTitle').textContent = CONFIG.introduce.title;
  ...
  function showLoadingPlaceholders() {
    ...
    const introducePhotos = $('#introducePhotos');
    ...
    if (introducePhotos) introducePhotos.innerHTML = placeholderHTML;
  ...
  async function init() {
    ...
    const [introduceImages, galleryImages] = await Promise.all([
      loadImagesFromFolder('introduce'),
      loadImagesFromFolder('gallery')
    ]);

    initGreeting(introduceImages);
